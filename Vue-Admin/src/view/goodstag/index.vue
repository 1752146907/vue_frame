						...
						<Row class="tab-form-row"
							...
							<Col :xs="24"
								...
								<FormItem label="名称"
										  prop="goodsTagName"
										  :rules="[]">
									<Input v-model="search.goodsTagName"
										   type="text"
										   placeholder=""/>
								</FormItem>
							</Col>
								...
								render: (h, params) => {
									return h('div', [
										h('Button', {
											...
											on: {
												click: () => {
													this.handleEdit(params.row.goodsTagId);
												}
											}
										}, '修改')
									]);
...
<script>
	...
	export default {
		...
		mounted () {
			this.search = this.$store.state.goodsTag.search;
			this.pageIndex = this.$store.state.goodsTag.pageIndex;
			this.pageSize = this.$store.state.goodsTag.pageSize;
		...
		methods: {
			handleLoad: function () {
				...
				var data = this.$store.state.goodsTag.search;
				data = Object.assign(data, {
					pageIndex: this.$store.state.goodsTag.pageIndex,
					pageSize: this.$store.state.goodsTag.pageSize
				});

				this.request({
					url: '/ec/goods/tag/admin/v1/list',
					data: data,
					success: (response) => {
			...
			handleEdit: function (goodsTagId) {
				this.$router.push({
					path: '/ec/goods/tag/edit/' + goodsTagId,
					query: {}
				});
			},
			handleDelete: function () {
				...
				this.request({
					url: '/ec/goods/tag/admin/v1/delete',
					data: {
						goodsTagIdList: this.goodsTagIdList
					},
					success: (response) => {
			...
			handleConfirm: function () {
				if (this.goodsTagIdList.length == 0) {
			...
			handleSelectionChange: function (list) {
				...
				for (var item of list) {
					goodsTagIdList.push(item.goodsTagId);
				}
