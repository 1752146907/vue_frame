						...
						<Row class="tab-form-row"
							...
							<Col :xs="24"
								...
								<FormItem label="名称"
										  prop="goodsGroupName"
										  :rules="[]">
									<Input v-model="search.goodsGroupName"
										   type="text"
										   placeholder=""/>
								</FormItem>
							</Col>
								...
								render: (h, params) => {
									return h('div', [
										h('Button', {
											...
											on: {
												click: () => {
													this.handleEdit(params.row.goodsGroupId);
												}
											}
										}, '修改'),
										h('Button', {
											...
											on: {
												click: () => {
													this.handleAddSgroup(params.row.goodsGroupId);
												}
											}
										}, '添加商品')
									]);
...
<script>
	...
	export default {
		...
		mounted () {
			this.handleActiveMenu('/goods/group/index');
		...
		methods: {
			handleLoad: function () {
				...
				var data = this.$store.state.goodsGroup.search;
				data = Object.assign(data, {
					pageIndex: this.$store.state.goodsGroup.pageIndex,
					pageSize: this.$store.state.goodsGroup.pageSize,
					productId: this.productId
				});

				this.request({
					url: '/ec/goods/group/admin/v1/list',
					data: data,
					success: (response) => {
			...
			handleAddSgroup: function (goodsGroupId) {
				this.$router.push({
					path: '/goods/add/sgroup/' + goodsGroupId,
					query: {}
				});
			},
			handleEdit: function (goodsGroupId) {
				this.$router.push({
			...
			handleDelete: function () {
				...
				this.request({
					url: '/ec/goods/group/admin/v1/delete',
					data: {
						goodsGroupIdList: this.goodsGroupIdList
					},
					success: (response) => {
			...
			handleConfirm: function () {
				if (this.goodsGroupIdList.length == 0) {
			...
			handleSelectionChange: function (list) {
				var goodsGroupIdList = [];

				for (var item of list) {
					goodsGroupIdList.push(item.goodsGroupId);
				}

				this.goodsGroupIdList = goodsGroupIdList;
			},
